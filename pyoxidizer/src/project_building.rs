...
pub fn build_python_executable<'a>(
    ...
    let mut build_path = temp_dir.path().join("build");
    let mut artifacts_path = temp_dir.path().join("artifacts");
    ...
    // If the user has overridden the default generated Rust sources, copy them over the
    // autogenerated ones. Pointing project_path at the crate folder would seem more logical,
    // but it seems to cause problems importing filesystem modules at runtime.
    if let Ok(crate_root) = std::env::var("PYOXIDIZER_CUSTOM_RUST_SRC") {
        ...
        for fname in [
            ...
            "build.rs",
            "src/main.rs",
            // needs to match app name
            "app-manifest.rc",
            "app-icon.ico",
        ] {
            let (src, dst) = (crate_root.join(fname), project_path.join(fname));
            std::fs::copy(&src, &dst)
                .with_context(|| format!("copying {} to {}", src.display(), dst.display()))?;
        }

        // Redirect build and arifacts to crate folder for faster incremental builds.
        build_path = crate_root.join("build");
        artifacts_path = crate_root.join("artifacts");
    }
